...
from guardrail import fact_check
...
parser.add_argument(
    "-c", "--config", help="Provide a chatbot config to run the deployment"
)
                ...
                with st.expander(os.path.basename(key)):
                    ...
                    if "source" in source["doc_metadata"]:
                        ...
                        if "page" in source_str and "block" in source_str:
                            download_path = (
                                source_str.split("page")[0].strip("-") + ".pdf"
                            )
                            ...
                            try:
                                ...
                                st.download_button(
                                    "Download now",
                                    f,
                                    key=download_path + str(n) + str(ctr),
                                    file_name=file_name,
                                )
                            except:
                                st.write(
                                    "failed to provide download for this file: ",
                                    file_name,
                                )
                            ...
                            download_path = os.path.join(
                                "vectorstore/ppt_references", ppt_path
                            )
                            file_name = os.path.basename(download_path)
                            f = open(download_path, "rb").read()
                            st.download_button(
...
if len(prompt) > 0 and submitted == True:
    ...
    if st.session_state.image_query:
        prompt = (
            f"\nI have uploaded an image with the following description: {st.session_state.image_query}"
